...
The quote is taken from [PHP-DI](https://php-di.org/doc/autowiring.html) library that we use.
...
This just means that you are not using dependency injection (as you have nothing to inject).
...
- You should create an interface with all the methods you need in the `DocumentsRoute` class, let's call it `QueryDocumentsInterface` and we'll put it inside the `src/Query/Documents` folder.
...
Let's provide you with some code examples:
